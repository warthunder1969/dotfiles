#!/bin/bash
#Warthunder's Librewolf Browser installation script
#Version 1.1
...
case $CHOICE in
    1)
		...
		dpkg -l | grep -qw extrepo || sudo apt install -yyq extrepo
		...
		if dpkg-query -W -f='${Status}' "librewolf" 2>/dev/null | grep -q "ok installed"; then
